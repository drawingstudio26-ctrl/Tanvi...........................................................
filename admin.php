            ...
            <div class="tab-content <?= $current_tab == 'promotions' ? 'active' : '' ?>" id="promotions">
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php 
                            $subscriptions = $pdo->query("SELECT * FROM subscriptions ORDER BY subscription_date DESC LIMIT 5")->fetchAll();
                            foreach ($subscriptions as $sub): ?>
                            <tr>
                                <td><?= htmlspecialchars($sub['customer_name']) ?></td>
                                <td><?= htmlspecialchars($sub['customer_email']) ?></td>
                                <td><?= ucfirst($sub['subscription_type']) ?></td>
                                <td><?= date('d M Y', strtotime($sub['subscription_date'])) ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php 
                            $coupons = $pdo->query("SELECT * FROM coupon_codes ORDER BY valid_until DESC")->fetchAll();
                            
                            if (empty($coupons)): ?>
                                <tr>
                                    <td colspan="6" class="text-center">No coupon codes found</td>
                                </tr>
                            <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-action btn-primary edit-coupon-btn" 
                                                data-coupon-id="<?= $coupon['id'] ?>" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <form action="admin.php" method="POST" style="display: inline;">
                                            <input type="hidden" name="coupon_id" value="<?= $coupon['id'] ?>">
                                            <button type="submit" name="delete_coupon" class="btn btn-sm btn-action btn-danger" 
                                                    title="Delete" onclick="return confirm('Are you sure you want to delete this coupon?')">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </form>
                                    </td>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.view-order-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    // Fetch actual order data via AJAX
                        ...
                        .then(data => {
                            document.getElementById('orderDetailsContent').innerHTML = `
                                ...
                                <div class="mb-3">
                                    ...
                                    <strong>Price:</strong> ₹${data.product_price.toFixed(2)}<br>
                                    <strong>Total:</strong> ₹${(data.product_price * data.quantity).toFixed(2)}
                                </div>
                                <div class="mb-3">
                                    <strong>Order Date:</strong> ${new Date(data.purchase_date).toLocaleDateString()}<br>
                                    <strong>Status:</strong> <span class="badge bg-${data.status === 'Completed' ? 'success' : data.status === 'Pending' ? 'warning' : 'danger'}">${data.status}</span><br>
            ...
            // View customer button click handler - shows real customer data
            document.querySelectorAll('.view-customer-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const row = this.closest('tr');
                    const customerName = row.querySelector('td:nth-child(1) strong').textContent;
                    ...
                    const customerOrders = row.querySelector('td:nth-child(4)').textContent;
                    const customerSpent = row.querySelector('td:nth-child(5)').textContent;

                    // Get the customer's recent orders from the database
                    fetch('get_customer_orders.php?email=' + encodeURIComponent(customerEmail))
                        .then(response => response.json())
                        .then(orders => {
                            document.getElementById('orderDetailsContent').innerHTML = `
                                ...
                                <div class="mb-3">
                                    <strong>Total Orders:</strong> ${customerOrders}<br>
                                    <strong>Total Spent:</strong> ${customerSpent}
                                </div>
                                        ...
                                        <tbody>
                                            ${orders.map(order => `
                                                <tr>
                                                    ...
                                                    <td>₹${(order.product_price * order.quantity).toFixed(2)}</td>
                        ...
                        .catch(error => {
                            console.error('Error:', error);
                            // Fallback to showing basic info if AJAX fails
                            document.getElementById('orderDetailsContent').innerHTML = `
                                <h6>Customer Details</h6>
                                <div class="mb-3">
                                    <strong>Name:</strong> ${customerName}<br>
                                ...
                                <div class="alert alert-warning">Could not load recent orders</div>
                            `;
                            viewOrderModal.show();
                        });
                });
                    ...
                    fetch('get_order_data.php?id=' + orderId)
                        ...
                        .then(data => {
                            document.getElementById('editOrderContent').innerHTML = `
                                ...
                                <div class="mb-3">
                                    <label class="form-label">Customer Email</label>
                                    <input type="email" name="customer_email" class="form-control" value="${data.customer_email}" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Customer Phone</label>
                                    <input type="tel" name="customer_phone" class="form-control" value="${data.customer_phone}" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Product Name</label>
                                ...
                                <div class="mb-3">
                                    <label class="form-label">Product Price</label>
                                    <input type="number" step="0.01" name="product_price" class="form-control" value="${data.product_price}" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Quantity</label>
                        ...
                        .catch(error => {
                            console.error('Error:', error);
                            document.getElementById('editOrderContent').innerHTML = `
                                <div class="alert alert-danger">Error loading order data</div>
                            `;
                            editOrderModal.show();
                        });
                });
