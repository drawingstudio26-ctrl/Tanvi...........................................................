...
function loadDeletedInquiries() {
  ...
  // Display count
  document.getElementById('deletedShowingCount').textContent = filtered.length;
...
// Delete inquiry (move to deleted)
...
let currentPage = 1;
const itemsPerPage = 5; // Change this to show more/fewer items per page

// Update the loadInquiries function with pagination
function loadInquiries() {
  const enquiries = JSON.parse(localStorage.getItem('enquiries'));
  const inquiryList = document.getElementById('inquiryList');
  const searchTerm = document.getElementById('searchInquiries').value.toLowerCase();
  ...
  // Display count
  document.getElementById('showingCount').textContent = `${startIndex + 1}-${Math.min(startIndex + itemsPerPage, filtered.length)}`;
...
document.getElementById('searchInquiries').addEventListener('input', function() {
  currentPage = 1;
  loadInquiries();
});
...
document.getElementById('searchInquiries').addEventListener('input', loadInquiries);
document.getElementById('filterStatus').addEventListener('change', loadInquiries);
document.getElementById('searchDeletedInquiries').addEventListener('input', loadDeletedInquiries);
